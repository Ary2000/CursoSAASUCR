...
def crearTorneo(body: TorneoCreate) -> Torneo:
    ...
    data={
        ...
        "eventoId": torneo.eventoId,
        "message": "Torneo creado"
    }

    rabbtmq_host = os.getenv('RABBITMQ_HOST')


    connection = pika.BlockingConnection(
        pika.ConnectionParameters(rabbtmq_host))
    
    channel = connection.channel()
    channel.queue_declare(queue='torneos_queue', durable=True)

    channel.basic_publish(exchange='', routing_key='torneos_queue', body=str(data))
    connection.close()
    logger.info(f"Torneo creado: {data}")
...
def borrarTorneo(
    ...
    for i, user in enumerate(TORNEOS):
        if user.id == user_id:
            torneoEliminado = TORNEOS[i]
            TORNEOS.pop(i)

            data={
                "id": torneoEliminado.id,
                "nombre": torneoEliminado.nombre,
                "eventoId": torneoEliminado.eventoId,
            ...
            channel.basic_publish(exchange='', routing_key='torneos_queue', body=str(data))
            connection.close()
            logger.info(f"Torneo eliminado: {data}")
...
def verJuego(id: int = Path(..., description="Identificador del juego a obtener")) -> Juego:
    response = requests.get(f"{os.getenv('CATALOGO_JUEGOS_SERVICE')}/games/{id}")
...
def listarJuegos() -> list[Juego]:
    response = requests.get(f"{os.getenv('CATALOGO_JUEGOS_SERVICE')}/games").json()
