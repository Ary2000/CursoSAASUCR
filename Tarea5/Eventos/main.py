...
class Query:
    ...
    def listarTorneosEvento(self, idEvento: int) -> list[Torneo]:        
        response = requests.get(f"{os.getenv('TORNEOS_SERVICE')}/evento/{idEvento}", headers=downstream_headers())
        if response.status_code != 200:
            return []
        data = response.json()  # list of tournament IDs
        torneos = []
        for torneo_id in data:
            response2 = requests.get(f"{os.getenv('TORNEOS_SERVICE')}/tourneys/{torneo_id}", headers=downstream_headers())
            if response2.status_code == 200:
                torneo_data = response2.json()
                if 'fechaHoraInicio' in torneo_data and torneo_data['fechaHoraInicio']:
                    torneo_data['fechaHoraInicio'] = datetime.fromisoformat(torneo_data['fechaHoraInicio'].replace('Z', '+00:00'))
                if 'fechaHoraFin' in torneo_data and torneo_data['fechaHoraFin']:
                    torneo_data['fechaHoraFin'] = datetime.fromisoformat(torneo_data['fechaHoraFin'].replace('Z', '+00:00'))
    ...
    def tournament_participants(self, idTorneo: int) -> list[TournamentParticipant]:
        response = requests.get(f"{os.getenv('TORNEOS_SERVICE')}/tourneys/{idTorneo}/participants", headers=downstream_headers()).json()
...
class Mutation:
    ...
    def registrarParticipante(self, idUsuario: int, idEvento: int) -> str:
        for p in PARTICIPANTES:
            if p["idUsuario"] == idUsuario and p["idEvento"] == idEvento:
                raise GraphQLError("El usuario ya está inscrito en el evento.")
    ...
    def eliminarParticipante(self, idUsuario: int, idEvento: int) -> str:
        ...
        raise GraphQLError("El usuario no estaba inscrito en el evento.")
    ...
    def crearTorneo(self, idEvento: int, nombre: str, fechaHoraInicio: datetime, fechaHoraFin: datetime, tipo: int, premio: str) -> Torneo:
        response = requests.post(f"{os.getenv('TORNEOS_SERVICE')}/tourneys", headers=downstream_headers(), json={
            "eventoId": idEvento,
            "nombre": nombre,
        ...
        if response.status_code == 404:
            raise GraphQLError("El evento no existe.")
        data = response.json()
        return Torneo(**data)
    ...
    def modificarTorneo(self, idTorneo: int, nombre: str | None = None, fechaHoraInicio: datetime | None = None, fechaHoraFin: datetime | None = None, tipo: int | None = None, premio: str | None = None) -> Torneo:
        response = requests.put(f"{os.getenv('TORNEOS_SERVICE')}/tourneys/{idTorneo}", headers=downstream_headers(), json={
            "nombre": nombre,
        ...
        if response.status_code == 404:
            raise GraphQLError("El torneo no existe.")
        data = response.json()
    ...
    def eliminarTorneo(self, idTorneo: int) -> str:
        response = requests.delete(f"{os.getenv('TORNEOS_SERVICE')}/tourneys/{idTorneo}", headers=downstream_headers())
        if response.status_code == 404:
            raise GraphQLError("El torneo no existe.")
